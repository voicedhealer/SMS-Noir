...
> comme archive. C'est ce fichier que `scripts/verify-fidelity.py` compare à la base.
>
> **Corrections apportées à la rédaction d'origine**, validées par Vivien :
> - Comptages : **21 blocs de micro-choix** (et non 22) et **11 choix
>   structurants** (et non 12). L'amplitude mono-axe est donc de 21 choix, pas 22.
> - Paliers d'`enquete` : **2 et 6** (et non 4 et 8). Sur trois axes, 50 %
>   d'enquête est déjà une nette dominante ; le palier bas est ce qui rend la
>   rejouabilité visible.
> - Effets : la mention `+0,5` par axe est remplacée par une **proportion**.
>   Un incrément fixe saturait `lucidite` (max 5) à mi-chapitre et rendait tous
>   les choix suivants inertes. Voir `docs/LOGIQUE.md § La grammaire des trois axes`.
> - Incohérence n°1 : le récépissé est daté du **12 juin**.
...
|---|---|---|---|
| 🛡 **Protéger** | Il s'inquiète pour elle, la met en garde, la rassure | proportion → `confiance` | Fins « la sauver » |
| 🔍 **Enquêter** | Il creuse, demande des détails, cherche avec elle | proportion → `enquete` | Options d'enquête ch. 4-5 |
| 🧠 **Raisonner** | Il prend du recul, doute, questionne la logique | proportion → `lucidite` | Fin cachée |
...
2. **Les effets sont une proportion, jamais un incrément.** C'est le **motif** sur tout le chapitre qui compte, pas l'occurrence — et aucun choix n'est jamais inerte, y compris le dernier.
...
**Nouvelle variable :** `enquete` (0-10, départ 0). Aux paliers 2 et 6, elle débloque des options d'enquête aux ch. 4-5. Distincte de `indices` (liste d'objets trouvés) : `enquete` mesure la posture, `indices` les découvertes.
...
| Interventions du joueur | 12 | 11 structurants + 21 micro = 32 |
| Délai max hors N19 | 90s | 25s |
| Silence N19 | 90s | 60s |
| Grammaire des micro-choix | — | 3 axes constants |

**Amplitude des variables sur le chapitre :** la posture est une **proportion**, pas un compteur. Un mono-axe pur atteint le maximum de son axe, un joueur équilibré n'obtient rien sur aucun, et un joueur à 80 % sur un axe obtient le même résultat qu'il ait vu 20 ou 60 micro-choix. C'est le motif qui compte, littéralement.
...
- Effets : `{"motif": "proteger" | "enquete" | "raison"}` — **le contenu ne porte aucun nombre**. Les ⚠️ portent un effet supplémentaire, lui chiffré.
